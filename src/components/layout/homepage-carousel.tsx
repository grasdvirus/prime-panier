...
const DotButton = ({ selected, onClick }: { selected: boolean, onClick: () => void }) => (
  <button
    className={cn(
      "h-2 rounded-sm bg-muted transition-all duration-300 hover:bg-muted-foreground/50",
      ...
      <div className="overflow-hidden" ref={emblaRef}>
        <div className="flex -ml-4">
          {slides.map((slide, index) => (
            <div 
                className="flex-[0_0_90%] md:flex-[0_0_85%] pl-4"
                key={slide.id}
            >
              <div className={cn("transition-all duration-500", index === selectedIndex ? "" : "blur-sm scale-95 opacity-80")}>
       ...
       <div className="absolute left-0 sm:left-4 top-1/2 -translate-y-1/2 z-20">
          <Button variant="glass" size="icon" onClick={scrollPrev}>
            <ArrowLeft className="h-5 w-5" />
          </Button>
      </div>
      <div className="absolute right-0 sm:right-4 top-1/2 -translate-y-1/2 z-20">
